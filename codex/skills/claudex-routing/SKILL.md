...
- `claudex-luna`: use whenever breadth is the cost: wide read-only inventories, maps, structured data analysis, validation anomalies, naming/stale-reference audits, log/test/diff classification, broad diff-risk maps, and compressed evidence before Sol, implementation, or Sol Review.
...
- `claudex-sol-review`: read-only PASS / CHANGES_REQUIRED / BLOCKED gate only for consequential, release-critical, experimental/non-primary implementation, or truly substantial/sensitive work; not for tiny, bounded, reversible, non-sensitive edits or routine Sol-backed implementation with clear validation.
