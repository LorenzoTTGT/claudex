...
- Use Claudex Luna at medium effort whenever breadth is the cost: repository inventories, call-site or dependency maps, data-quality analysis, classification, naming/stale-reference audits, failed-log grouping, broad diff-risk maps, and compressed evidence summaries before Sol, implementation, or Sol Review. Luna never replaces architecture judgment or review gates.
...
- Use `claudex-sol-review` at medium effort only for consequential decisions involving security, authentication, privacy, permissions, payments, data integrity, migrations, deployment, public APIs, storage/schema, integrations, or cross-service changes; for release-critical work; for experimental/non-primary implementation scrutiny; and for truly substantial or sensitive diffs. Do not use it for tiny, bounded, reversible, non-sensitive edits or routine Sol-backed implementation with clear validation.
