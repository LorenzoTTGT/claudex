...
Delegate only bounded side work, except that the Sol planning and architecture advisory below is mandatory whenever its trigger applies:
...
Treat an explicit user request containing `plan`, `planning`, `architecture`, or `architectural` as a direct Sol trigger unless the user is clearly negating that action (for example, “do not make a plan”). Before the coordinator finalizes, presents, approves, or reviews any implementation plan, it must invoke `claudex-sol` for scoped read-only feedback. Before it makes, presents, approves, or reviews any architecture choice, it must invoke `claudex-sol`. This is the default even when the coordinator could plan or decide directly. An architecture choice is a design decision among credible structural, contract, data-flow, storage, or dependency alternatives; routine mechanical implementation choices do not count.
...
Do not infer that a Claude Code/Claudex session is dead because it has been quiet, produced a short summary, emitted a hook event, or appears stopped in a local agent view. Hooks, notifications, and `claude agents --json` are not standalone authoritative resume signals. Do not implement or install session monitoring, lifecycle observation machinery, agent-view polling, process watchers, session-state persistence, automatic resume, restart, or steering from those signals. The narrow durable accounting exceptions are opt-in, content-free, invocation-scoped launcher telemetry and explicitly user-invoked retrospective usage-efficiency snapshots: they may record only aggregate launcher observations or aggregate quota-window usage/duration metrics, never prompts, transcripts, raw paths, session/thread IDs, lifecycle inference, polling, resume, restart, steering, or external sends.
