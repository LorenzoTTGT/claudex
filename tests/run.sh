...
grep -Fq 'Before the root finalizes, presents, approves, or reviews any non-trivial implementation plan' "$ROOT_DIR/codex/AGENTS.md" || fail 'Codex AGENTS policy must require Sol for non-trivial implementation plans'
...
grep -Fq 'Use `claudex-sol` at medium effort for scoped feedback on real implementation plans' "$ROOT_DIR/prompts/terra-routing.md" || fail 'routing prompt must define Sol as a scoped medium-effort advisory'
grep -Fq 'Use `claudex-implementer` on GPT-5.6 Sol at low effort for substantial bounded non-frontend implementation' "$ROOT_DIR/prompts/terra-routing.md" || fail 'routing prompt must define the Sol-low implementer boundary'
...
grep -Fq 'Before the coordinator finalizes, presents, approves, or reviews a non-trivial implementation plan' "$ROOT_DIR/prompts/terra-routing.md" || fail 'routing prompt must require Sol for non-trivial implementation plans'
...
grep -Fq 'Sol advisory and GPT-5.6 Sol implementation never replace either review gate' "$ROOT_DIR/prompts/terra-routing.md" || fail 'routing prompt must keep Sol advisory and implementation separate from Sol Review gates'
...
grep -Fq 'Frontend work never substitutes for a required risk-based `claudex-sol-review`.' "$ROOT_DIR/prompts/terra-routing.md" || fail 'routing prompt must retain risk-based Sol Review gates for Frontend work'
...
grep -Fq 'Use `claudex-sol` at medium effort for scoped feedback on non-trivial implementation plans' "$ROOT_DIR/agents/claudex-terra.md" || fail 'Terra agent must define Sol as a scoped medium-effort advisory'
grep -Fq 'Use `claudex-implementer` on GPT-5.6 Sol at low effort for substantial, bounded non-frontend implementation' "$ROOT_DIR/agents/claudex-terra.md" || fail 'Terra agent must define the Sol-low implementer boundary'
...
grep -Fq 'Before you finalize, present, approve, or review any non-trivial implementation plan' "$ROOT_DIR/agents/claudex-terra.md" || fail 'Terra agent must require Sol for non-trivial implementation plans'
...
grep -Fq 'the Sol advisory and GPT-5.6 Sol-backed implementation are additional to and never satisfy or replace either independent `claudex-sol-review` gate' "$ROOT_DIR/agents/claudex-terra.md" || fail 'Terra agent must keep Sol advisory and implementation separate from Sol Review gates'
...
grep -Fq 'Frontend implementation and tests never substitute for a required risk-based `claudex-sol-review`' "$ROOT_DIR/agents/claudex-terra.md" || fail 'Terra agent must retain risk-based Sol Review gates for Frontend work'
